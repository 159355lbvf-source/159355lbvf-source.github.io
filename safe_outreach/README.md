...
- `config/repos.txt` — готовый список из 20 репозиториев (`/workspace/agents/repo_01..20`).
- `config/accounts.csv` — готовый список из 20 аккаунтов (по 3 контакта в день).
- `config/contacts.csv` — рабочий файл контактов для планирования.
...
- `tools/run_daily.sh` — единый ежедневный запуск одной командой.
...
## Одна команда на каждый день

Из директории `safe_outreach`:

```bash
bash tools/run_daily.sh
```

Опционально:

- дата запуска: `bash tools/run_daily.sh 2026-03-14`
- изменить паузу между контактами (дни): `COOLDOWN_DAYS=3 bash tools/run_daily.sh`
- изменить команду проверки репозиториев: `CHECK_COMMAND="git pull --ff-only" bash tools/run_daily.sh`
